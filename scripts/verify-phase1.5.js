import { getDb, allocateBatchesFEFO, checkCreditLimit } from '../src/lib/db.ts';
import { checkPermission, AuthUser } from '../src/lib/auth.ts';
...
// Reset Product 1 Batches
const prodId = 'prod-001';
db.prepare('UPDATE products SET stock = 1500 WHERE id = ?').run(prodId);
db.prepare('UPDATE product_batches SET current_stock = 500 WHERE id = "bth-prod001-a1"').run();
db.prepare('UPDATE product_batches SET current_stock = 300 WHERE id = "bth-prod001-b2"').run();
db.prepare('UPDATE product_batches SET current_stock = 700 WHERE id = "bth-prod001-c3"').run();
...
const prodCheck = db.prepare('SELECT stock FROM products WHERE id = ?').get(prodId);
...
// Customer 1: credit limit 500,000, current outstanding 145,000
...
const initialBal = db.prepare('SELECT outstanding_balance FROM customers WHERE id = ?').get(testCustId).outstanding_balance;
...
const afterPayBal = db.prepare('SELECT outstanding_balance FROM customers WHERE id = ?').get(testCustId).outstanding_balance;
...
const revertedBal = db.prepare('SELECT outstanding_balance FROM customers WHERE id = ?').get(testCustId).outstanding_balance;
...
const salesUser = { id: 'usr-sales-003', email: 'sales@agrishield.in', full_name: 'Sales Exec', role: 'Sales Executive' };
const adminUser = { id: 'usr-admin-001', email: 'admin@agrishield.in', full_name: 'Admin', role: 'Admin' };
