...
export function parseAuthToken(request: Request): AuthUser | null {
  ...
  if (parts.length < 4 || parts[0] !== 'token' || parts[1] !== 'agrishield') {
    return null;
  }

  const userId = parts[2];
  const db = getDb();
  const user = db.prepare('SELECT id, email, full_name, role, is_active FROM users WHERE id = ? AND is_active = 1').get(userId) as any;
...
export function checkPermission(
  ...
  if (!user) {
    return { allowed: false, error: 'UNAUTHORIZED: Authentication token is missing or invalid' };
  }
  ...
  if (user.role === 'Warehouse Manager') {
    if (resource === 'inventory' || resource === 'products') {
      if (action === 'DELETE') {
        return { allowed: false, error: 'FORBIDDEN: Warehouse Managers cannot delete master catalog records' };
      }
      return { allowed: true };
    }
  ...
  if (user.role === 'Accountant') {
    ...
    if (resource === 'users') {
      return { allowed: false, error: 'FORBIDDEN: Accountants cannot manage users' };
    }
    return { allowed: true };
  }
  ...
  if (user.role === 'Sales Executive') {
    ...
    if (action === 'DELETE') {
      return { allowed: false, error: 'FORBIDDEN: Sales Executives are not authorized to perform delete operations' };
    }
    if (resource === 'settings') {
      return { allowed: false, error: 'FORBIDDEN: Sales Executives cannot view or modify company settings' };
    }
    if (resource === 'users') {
      return { allowed: false, error: 'FORBIDDEN: Sales Executives cannot manage user roles' };
    }
    return { allowed: true };
  }
